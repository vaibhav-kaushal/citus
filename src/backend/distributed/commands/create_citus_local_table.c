...
CreateCitusLocalTable(Oid relationId, bool cascade)
{
	...
	if (tableHasExternalForeignKeys && cascade)
	{
		ExecuteCitusTableFunctionCascade(relationId, lockMode, CREATE_CITUS_LOCAL_TABLE);
	...
	if (tableHasExternalForeignKeys)
	{
		...
		ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						errmsg("relation \"%s\" is involved in a foreign key "
							   "relationship with another table", relationName),
