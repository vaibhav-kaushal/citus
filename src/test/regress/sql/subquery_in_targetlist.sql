...
SELECT (SELECT DISTINCT user_id FROM users_reference_table WHERE user_id IN (SELECT user_id FROM users_table) AND user_id < 2)
FROM users_reference_table r
ORDER BY 1 LIMIT 3;

-- recurring from clause with sublink with distributed table in sublink in target list
SELECT (SELECT (SELECT user_id FROM users_table WHERE user_id < 2 GROUP BY user_id)
